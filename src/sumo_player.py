...
class SumoPlayer(AbstractPlayer):
    ...
    def start(self, freq_in_ms):
        ...
        print("SUMO is starting with ratio: " +
              str(self.step_ratio) + " and " + self.config_name)
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            def lc(s): return s[:1].lower() + s[1:] if s else ''
            ...
            for veh_id in vehicles_ids:
                ...
                replaceValues["$ObsID$"] = "O_" + \
                    str(veh_id) + "-" + str(self.step)
                ...
                if veh_laneID in self.lane_mappings:
                    # (sumo id, asp id, asp orientation)
                    laneTuple = self.lane_mappings[veh_laneID]
                ...
                veh_lane = str(traci.vehicle.getLaneID(veh_id)) + ";" + \
                    str(round(traci.vehicle.getLanePosition(veh_id), 4))
            ...
            for intersID in tl_id_list:
                ...
                if(intersID in self.rows_pos):
                    ...
                    # row_pos is at tuple (tlid positions)
                    for rowPosDict in row_pos_values:
                ...
                if(hasChildTemplate):
                    msg2 = msg2.replace(
                        self.TEMPL_PLACEHOLDER_CHILDS, str(msg2_Childs))
                    yield msg2
        ...
        for line1 in linesBasePlan:  # f_open1:
            ...
            if line1.find("map") >= 0:
                ...
                if(predName == "lmap"):
                    ...
                    lineTuple = (landIdSumo, lineDataSplit[1].strip(
                    ), lineDataSplit[2].strip())  # sumo_id, asp_id, asp_dir
...
def main(argv):
    ...
    templates = {}
    # json
    templates["subStreamVehicles"] = "stream-templates/traffic/vehicle_template.asp"
    # json
    templates["subStreamTrafficLights"] = "stream-templates/traffic/traffic_light_template.asp"
