...
class Observation:
    ...
    def get_graph(self, graph):
        ...
        for result in self.results:
            ...
            detection_result_iri = URIRef(SR_NAMESPACE + '/detection/' + self.id + '/' + str(result_id))
            box_result_iri = URIRef(SR_NAMESPACE + '/box/' + self.id + '/' + str(result_id))
            detected_object = URIRef(SR_NAMESPACE + '/detectedObject/' + label + '/')

            graph.add((observation_iri, SOSA.hasResult, detection_result_iri))
            graph.add((detection_result_iri, RDF.type, detection_class_iri))
            graph.add((detection_result_iri, has_detected_object, detected_object))
            graph.add((detection_result_iri, has_box, box_result_iri))
            graph.add((detected_object, RDFS.label, Literal(label)))

            graph.add((box_result_iri, box_x, Literal(bbox[0], datatype=XSD.float)))
            graph.add((box_result_iri, box_y, Literal(bbox[1], datatype=XSD.float)))
            graph.add((box_result_iri, box_width, Literal(bbox[2], datatype=XSD.float)))
            graph.add((box_result_iri, box_height, Literal(bbox[3], datatype=XSD.float)))
...
def main():
    player = ObjectDetection('stream-log-files/kitti/yolov4-2011_09_26_drive_0015-1.json', 'n3')
    player.start(1)
